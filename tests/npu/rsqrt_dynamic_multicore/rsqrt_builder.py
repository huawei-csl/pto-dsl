...
def build_rsqrt(fn_name="rsqrt_float32", dtype="float32"):
    """
    Dynamic multicore rsqrt kernel.

    Args:
        x_ptr     : dtype[batch * n_cols]  input matrix, row-major (elements must be positive)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fn-name", default="rsqrt_float32")
    parser.add_argument("--dtype", choices=list(_DTYPE_BYTES), default="float32")
    args = parser.parse_args()
    print(build_rsqrt(fn_name=args.fn_name, dtype=args.dtype))
