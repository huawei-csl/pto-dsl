...
def build_geglu(fn_name="geglu_fp16"):
    """
    Build a dynamic-batch GEGLU kernel in PTO DSL (optimized).

    Computes c = gelu_approx(a) * b, where:
        gelu_approx(a) = a * sigmoid(2a)
        sigmoid(x)     = exp(x) / (exp(x) + 1)

    Optimizations vs baseline:
      1. Sigmoid reformulation: gelu(a) = a * sigmoid(2a) uses only
         1 exp + 1 div per row instead of 2 exp + 2 div.
      2. Hoisted ones: exp(a-a)=1.0 computed once before the loop.

    UB tile budget (fp16, 5 tiles x 32 KB = 160 KB < 192 KB):
        tb_a    : input row a
        tb_b    : input row b
        tb_ones : constant 1.0 (computed once before loop)
            ...
            with pto.if_context(n_cols > c0):
                with pto.if_context(c_tile >= n_cols):
                    ...
                    with pto.if_context(num_rows > c0):
                        ...
                        # Hoist ones computation: load first row, compute exp(a-a)=1
                        sv_a_first = pto.slice_view(
                            subtensor_type,
                            source=tv_a,
                            offsets=[row_start * n_cols],
                            sizes=[n_cols],
                        )
                        pto.load(sv_a_first, tb_a)
                        pto.sub(tb_a, tb_a, tb_tmp1)  # tmp1 = 0
                        pto.exp(tb_tmp1, tb_ones)  # ones = 1.0
                        ...
                        for row_i in pto.for_range(c0, num_rows, c1):
                            ...
                            # gelu(a) = a * sigmoid(2a)
                            # sigmoid(2a) = exp(2a) / (exp(2a) + 1)
                            pto.add(tb_a, tb_a, tb_tmp1)  # tmp1 = 2a
                            pto.exp(tb_tmp1, tb_tmp1)  # tmp1 = exp(2a)
                            pto.add(tb_tmp1, tb_ones, tb_tmp2)  # tmp2 = exp(2a) + 1
                            pto.div(tb_tmp1, tb_tmp2, tb_tmp1)  # tmp1 = sigmoid(2a)

                            # GEGLU: c = a * sigmoid(2a) * b
                            pto.mul(tb_a, tb_tmp1, tb_tmp1)  # tmp1 = gelu(a)
