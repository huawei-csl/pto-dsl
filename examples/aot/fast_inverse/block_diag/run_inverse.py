...
def run_kernel(lib, inp, blockdiag_size=16):
    ...
    identity_neg = torch.zeros((n, n), dtype=torch.float16, device=inp_fp16.device)
...
def check_case(
    ...
    for block_dim_x in block_dim_x_list:
        for block_dim_y in block_dim_y_list:
            ...
            out = run_kernel(lib, inp, blockdiag_size=blockdiag_size).cpu().to(torch.float64)
    ...
    total = len(block_dim_x_list) * len(block_dim_y_list)
    print(f"summary: n={n}, pass={passes}, fail={len(failures)}, total={total}")
    return failures


def run_test(lib, n: int):
    failures = []
    if n >= 16:
        failures.extend(
            check_case(
