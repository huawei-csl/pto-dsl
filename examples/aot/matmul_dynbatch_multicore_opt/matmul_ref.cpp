...
__global__ AICORE void RunTMATMULSplitK(__gm__ half* v1, __gm__ half* v2, __gm__ half* v3, __gm__ half* v4, bool v5, int32_t v6) {
  ...
  using GMType = GlobalTensor<half, pto::Shape<1, 1, 1, 128, 128>, pto::Stride<16384, 16384, 16384, 128, 1>, pto::Layout::ND>;
  ...
  // i-1
  // must load the first tile from GM->l1 here since the loop always loads for next iteration
  ...
  for (size_t i = v24; i < end; i += 1) {
    curr = i & 1;
    // Global memory A_1 and A_2
    GMType v41 = GMType(v2 + i * v15, v39, v40);
    GMType v45 = GMType(v2 + (i+1) * v15, v43, v44);
    // GM tile C_1 and C_2
    GMType v48 = GMType(v1 + i * v15, v46, v47);
    GMType v51 = GMType(v1 + (i) * v15, v49, v50);
    ...
    // mov
    wait_flag(PIPE_MTE2, PIPE_MTE1, 1-curr); // (2, i-1) last iteration loaded the tile into l1, so for us to move to l0 we wait for last it
    wait_flag(PIPE_M, PIPE_MTE1, curr); // (3, i-2) make sure the matmul from i-2 finished so we can overwrite l0A
    ...
    // matmul
    wait_flag(PIPE_FIX, PIPE_M, curr); // (4, i-2) wait until the STORE at it. i-2 has written back from L0C
    ...
    set_flag(PIPE_M, PIPE_FIX, curr); // set(6, i) notify store in this iteration i, that matmul is done
    if (i+2<end) {
    ...
    TSTORE(curr==0?v48:v51, curr==0?C1_l0:C2_l0);
    if (i+2<end) {
