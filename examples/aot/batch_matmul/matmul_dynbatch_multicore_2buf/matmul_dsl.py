...
def build(M=128, K=128, N=128):
    ...
    def RunTMATMULSplitK(
        ...
        with pto.cube_section():
            ...
            # Put B in L0B
            svB = pto.slice_view(
                tile_view_b, source=tvB, offsets=[c0, c0], sizes=[cK, cN]
            )
            ...
            svA = pto.slice_view(
                tile_view_a, source=tvA, offsets=[b_start, c0, c0], sizes=[c1, cM, cK]
            )
            curr = c1 - (b_start % c2)
            pto.cond(
                curr == c1,
            ...
            for b_idx in pto.range(b_start, b_end, c1):
                ...
                svC = pto.slice_view(
                    tile_view_c, source=tvC, offsets=[b_idx, c0, c0], sizes=[c1, cM, cN]
                )
