...
#
# Compile/run parity: FA_NUM_TILES, FA_S1_TILE, and FA_Q_ROWS are baked into the
# emitted MLIR at compile time. Each compile emits matching fa*.build_env next to
# the .so; run.py applies the sidecar for the first FA_BENCH_LENGTHS entry and
# reloads fa_performance_builder (see README). Override FA_* in the environment
# if you skip compile.sh sidecars or resolve ambiguous seq_len factorizations.
#
...
import fa_performance_builder as fb  # noqa: E402 — reloaded in main() after build_env sync
...
# Each must be a multiple of S1_TILE (from reload/synced fa_performance_builder)
# and have a matching prebuilt .so.
...
def attn_flops_matmul_softmax_scale(
    ...
    """Same FLOP model as `cpp_ref/split_pipe/run.py` for comparable TFLOP/s."""
...
def get_block_dim() -> int:
    return min(fb.NUM_Q_BLOCKS, get_num_cube_cores())


def get_slot_elems(block_dim: int) -> int:
    return fb.GM_ELEMS_PER_BLOCK * block_dim
...
def _apply_build_env_matching_seq(seq_len: int) -> None:
    """Set FA_* env vars from the fa*.build_env written by compile.sh for this seq_len."""
    if not os.path.isdir(ARTIFACT_DIR):
        return
    matches = []
    for fn in sorted(os.listdir(ARTIFACT_DIR)):
        if not fn.endswith(".build_env"):
            continue
        ...
        try:
            ...
            continue
        try:
            nt = int(kv["FA_NUM_TILES"])
            s1 = int(kv["FA_S1_TILE"])
        except (KeyError, ValueError):
            continue
        if nt * s1 != seq_len:
            continue
        matches.append((fn, kv))

    if not matches:
        return

    # Same seq_len can factor two ways (e.g. 8192 = 16×512 = 32×256): prefer
    # FA_S1_TILE from the environment when set; else prefer canonical fa.build_env.
    env_s1 = os.environ.get("FA_S1_TILE")
    if env_s1 is not None:
        for fn, kv in matches:
            if kv.get("FA_S1_TILE") == env_s1.strip():
                for k, v in kv.items():
                    os.environ[k] = v
                return

    matches.sort(key=lambda x: (0 if x[0] == "fa.build_env" else 1, x[0]))
    for _, kv in matches:
        for k, v in kv.items():
            os.environ[k] = v
        return
...
def test_flash(lib, device, num_tiles):
    ...
    Q_ROWS = fb.Q_ROWS
    HEAD = fb.HEAD
    S1_TOTAL = fb.S1_TILE * num_tiles
    GM_ELEMS_PER_BLOCK = fb.GM_ELEMS_PER_BLOCK
...
def benchmark_flash(lib, device, num_tiles, warmup=10, iters=100):
    ...
    flops = attn_flops_matmul_softmax_scale(1, Q_ROWS, s1_total, HEAD)
    return {
        "seq_len": s1_total,
        "num_tiles": num_tiles,
        "block_dim": block_dim,
...
def plot_benchmark_results(results, out_png=None):
    ...
    fig.suptitle(
        f"Flash Attention Benchmark: Q={fb.Q_ROWS}, H={fb.HEAD}, "
        f"S1_TILE={fb.S1_TILE}"
    )
    fig.tight_layout()
...
def main():
    ...
    _apply_build_env_matching_seq(bench_lengths[0])
    ...
    print(f"\n{'Benchmark (fa)':=^96}")
    print(
        f"  Q_ROWS={fb.Q_ROWS}  HEAD={fb.HEAD}  "
        f"S1_TILE={fb.S1_TILE}  "
        f"NUM_Q_BLOCKS={fb.NUM_Q_BLOCKS}  cores={get_num_cube_cores()}"
    )
