...
build_variant() {
    ...
    FA_S1_TILE="${FA_S1_TILE:-512}" \
    FA_Q_ROWS="${FA_Q_ROWS:-3072}" \
        python "${SCRIPT_DIR}/kernels/fa_performance_builder.py" > "${mlir_path}"
    ...
    echo "    built ${lib_path}"
    {
        echo "FA_NUM_TILES=${num_tiles}"
        echo "FA_S1_TILE=${FA_S1_TILE:-512}"
        echo "FA_Q_ROWS=${FA_Q_ROWS:-3072}"
    } >"${ARTIFACT_DIR}/fa${tag}.build_env"
}

# Default tile set covers seqlen = NUM_TILES * FA_S1_TILE (FA_S1_TILE defaults to 512).
# Examples at FA_S1_TILE=512: 16 -> 8k, 32 -> 16k, 64 -> 32k, 128 -> 64k.
# FA_Q_ROWS defaults to 3072; must match python run env when invoking kernels.
