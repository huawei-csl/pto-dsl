"""Numerical-accuracy test for the deepseek_v4 ``fp4_gemm`` PTO kernel
(host-side scale-fusion design — see fp4_gemm_util.py)."""
...
from fp4_gemm_util import (  # noqa: E402
    BLOCK_K,
    BLOCK_M,
    BLOCK_N,
    _KERNEL_SO,
...
@pytest.mark.parametrize(
    "M,N,K",
    [
        (BLOCK_M, BLOCK_N, BLOCK_K * 4),
        (BLOCK_M * 2, BLOCK_N * 2, BLOCK_K * 8),
        (BLOCK_M * 4, BLOCK_N, BLOCK_K * 16),
    ],
)
def test_fp4_gemm_with_scales(npu_device, M, N, K):
    ...
    c_pto = fp4_gemm(a, b, sa, sb)
    c_ref = fp4_gemm_ref(a, b, sa, sb)
    torch.testing.assert_close(c_pto, c_ref, rtol=2e-2, atol=2e-2)


def test_fp4_gemm_unit_scales(npu_device):
    """Sa = Sb = 1 -> exact bare GEMM behavior."""
    torch.manual_seed(1)
    M, N, K = BLOCK_M * 2, BLOCK_N, BLOCK_K * 8
    a = (torch.randn(M, K, device=npu_device) * 0.1).to(torch.float16)
    b = (torch.randn(K, N, device=npu_device) * 0.1).to(torch.float16)
