"""PTO DSL port of TileLang fp4_gemm kernel.

Original (GPU): FP8 act × FP4 weight GEMM, A scaled per 128 on K, B scaled
per 32 on K. B stored as [N, K//2] fp4_e2m1fn_x2 (2 fp4 per byte, packed
along K).

NPU port: Same shape & blocking strategy but using FP16 inputs (no fp8/fp4
on Ascend). Asymmetric scale granularity (act per 128-K, weight per 32-K)
is preserved in the *interface* (Sa, Sb), but per-block scale fusion is
left as a TODO (see fp8_gemm/README.md). Block sizes:
block_M=32, block_N=128, block_K=32 (= weight_group_size).
"""
