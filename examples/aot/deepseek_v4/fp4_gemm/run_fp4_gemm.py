"""Run the deepseek_v4 ``fp4_gemm`` PTO kernel and validate against the
...
def _check(M, N, K, sa_unit: bool, device, seed: int):
    ...
    print(f"fp4_gemm M={M} N={N} K={K} {tag}: OK")
...
def main() -> int:
    ...
    _check(BLOCK_M * 2, BLOCK_N, BLOCK_K * 8, sa_unit=True, device=device, seed=42)
    print("fp4_gemm: all shapes PASSED")
    return 0
