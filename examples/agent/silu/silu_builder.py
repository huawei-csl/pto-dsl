...
def build_silu(fn_name="silu_fp16"):
    """
    Build a dynamic-batch SiLU (Swish) kernel in PTO DSL.

    Computes y = x * sigmoid(x), where:
        sigmoid(x) = 1 / (1 + exp(-x))
                ...
                with pto.if_context(c_tile >= n_cols):
                    ...
                    with pto.if_context(num_rows > c0):
                        ...
                        for row_i in pto.range(c0, num_rows, c1):
                            ...
                            # sigmoid(x) = 1 / (1 + exp(-x))
                            tile.sub(tb_tmp2, tb_x, tb_tmp1)  # tmp1 = 0 - x = -x
                            tile.exp(tb_tmp1, tb_tmp1)         # tmp1 = exp(-x)
                            tile.add(tb_tmp1, tb_ones, tb_tmp1) # tmp1 = 1 + exp(-x)
                            tile.div(tb_ones, tb_tmp1, tb_tmp1) # tmp1 = sigmoid(x)

                            # silu(x) = x * sigmoid(x)
                            tile.mul(tb_x, tb_tmp1, tb_tmp1)  # tmp1 = x * sigmoid(x)
...
if __name__ == "__main__":
    ...
    print(build_silu(fn_name=args.fn_name))
