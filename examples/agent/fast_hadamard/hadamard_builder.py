    ...
    with pto.vector_section():
        ...
        with pto.if_context(sample_offset < batch):
            ...
            with pto.if_context(samples_to_process > c0):
                ...
                # Single buffer set — no ping-pong overhead
                tb_row = pto.alloc_tile(tile_full, valid_col=n)
                tb_even = pto.alloc_tile(tile_half, valid_col=n // c2)
                tb_odd = pto.alloc_tile(tile_half, valid_col=n // c2)

                n_half = n // c2

                # Alias row halves inside UB row tile
                ...
                # Direct loop over samples — no chunk abstraction overhead
                for si in pto.range(c0, samples_to_process, c1):
                    row_offset = (sample_offset + si) * n
